...
class RVAdapter(private val datas: ArrayList<ResponseModel.Beritas>, val ctx:Context):RecyclerView.Adapter<RVAdapter.MyViewHolder>() {
    class  MyViewHolder(view:View):RecyclerView.ViewHolder(view){
        val binding = ItemsMainBinding.bind(view)
        val tvJudul = binding.tvJudul
        val tvKreator = binding.tvKreator
        val tvCreate = binding.tvCreateAt
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.tvCreate.text = datas[position].created_at
        Glide.with(holder.itemView.context).load("https://syafii-2520.herokuapp.com/"+datas[position].gambar_berita).circleCrop().into(holder.ivMain)
        holder.itemView.setOnClickListener {
            ...
            intent.putExtra("create",datas[position].created_at)
            intent.putExtra("gambar",datas[position].gambar_berita)
